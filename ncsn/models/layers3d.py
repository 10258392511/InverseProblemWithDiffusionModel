...
def conv1x1(in_planes, out_planes, stride=1, bias=True, spec_norm=False):
    "1x1 convolution"
    conv = nn.Conv3d(in_planes, out_planes, kernel_size=1, stride=stride,
                     padding=0, bias=bias)
    if spec_norm:
...
def conv3x3(in_planes, out_planes, stride=1, bias=True, spec_norm=False):
    ...
    conv = nn.Conv3d(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=1, bias=bias)
    if spec_norm:
        conv = spectral_norm(conv)
...
def stride_conv3x3(in_planes, out_planes, kernel_size, bias=True, spec_norm=False):
    conv = nn.Conv3d(in_planes, out_planes, kernel_size=kernel_size, stride=2,
...
def dilated_conv3x3(in_planes, out_planes, dilation, bias=True, spec_norm=False):
    conv = nn.Conv3d(in_planes, out_planes, kernel_size=3, padding=dilation, dilation=dilation, bias=bias)
    if spec_norm:
        conv = spectral_norm(conv)
...
class CRPBlock(nn.Module):
    def __init__(self, features, n_stages, act=nn.ReLU(), maxpool=True, spec_norm=False):
        ...
        if maxpool:
            self.maxpool = nn.MaxPool3d(kernel_size=5, stride=1, padding=2)
        else:
            self.maxpool = nn.AvgPool3d(kernel_size=5, stride=1, padding=2)
...
class ConvMeanPool(nn.Module):
    ...
    def forward(self, inputs):
        ...
        output = sum([output[:, :, ::2, ::2, ::2], output[:, :, ::2, ::2, 1::2],
                      output[:, :, ::2, 1::2, ::2], output[:, :, ::2, 1::2, 1::2],
                      output[:, :, 1::2, ::2, ::2], output[:, :, 1::2, ::2, 1::2],
                      output[:, :, 1::2, 1::2, ::2], output[:, :, 1::2, 1::2, 1::2]]) / 8.
...
class ConditionalResidualBlock(nn.Module):
    def __init__(self, input_dim, output_dim, num_classes, resample=None, act=nn.ELU(),
                 normalization=ConditionalBatchNorm3d, adjust_padding=False, dilation=None, spec_norm=False):
        super().__init__()
...
class ResidualBlock(nn.Module):
    def __init__(self, input_dim, output_dim, resample=None, act=nn.ELU(),
                 normalization=nn.BatchNorm3d, adjust_padding=False, dilation=None, spec_norm=False):
        super().__init__()
        self.non_linearity = act
        self.input_dim = input_dim
        self.output_dim = output_dim
