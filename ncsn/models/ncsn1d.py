...
class NCSN1D(nn.Module):
    def __init__(self, config):
        ...
        self.begin_conv = nn.Conv1d(config.data.channels, ngf, 3, stride=1, padding=1)

        self.normalizer = self.norm(ngf, self.num_classes)
        self.end_conv = nn.Conv1d(ngf, config.data.channels, 3, stride=1, padding=1)
