...
def conv1x1(in_planes, out_planes, stride=1, bias=True, spec_norm=False):
    "1x1 convolution"
    conv = nn.Conv1d(in_planes, out_planes, kernel_size=1, stride=stride,
                     padding=0, bias=bias)
    if spec_norm:
...
def conv3x3(in_planes, out_planes, stride=1, bias=True, spec_norm=False):
    ...
    conv = nn.Conv1d(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=1, bias=bias)
    if spec_norm:
        conv = spectral_norm(conv)
...
def stride_conv3x3(in_planes, out_planes, kernel_size, bias=True, spec_norm=False):
    conv = nn.Conv1d(in_planes, out_planes, kernel_size=kernel_size, stride=2,
...
def dilated_conv3x3(in_planes, out_planes, dilation, bias=True, spec_norm=False):
    conv = nn.Conv1d(in_planes, out_planes, kernel_size=3, padding=dilation, dilation=dilation, bias=bias)
    if spec_norm:
        conv = spectral_norm(conv)
...
class CRPBlock(nn.Module):
    def __init__(self, features, n_stages, act=nn.ReLU(), maxpool=True, spec_norm=False):
        ...
        if maxpool:
            ...
            self.maxpool = nn.AvgPool1d(kernel_size=5, stride=1, padding=2)
...
class MSFBlock(nn.Module):
    ...
    def forward(self, xs, shape):
        ...
        for i in range(len(self.convs)):
            ...
            h = F.interpolate(h, size=shape, mode='linear', align_corners=True)
            sums += h
        return sums
...
class MeanPoolConv(nn.Module):
    def __init__(self, input_dim, output_dim, kernel_size=3, biases=True, spec_norm=False):
        super().__init__()
        self.conv = nn.Conv1d(input_dim, output_dim, kernel_size, stride=1, padding=kernel_size // 2, bias=biases)
        if spec_norm:
    ...
    def forward(self, inputs):
        ...
        # output = sum([output[:, :, ::2, ::2], output[:, :, 1::2, ::2],
        #               output[:, :, ::2, 1::2], output[:, :, 1::2, 1::2]]) / 4.
        output = sum([output[:, :, ::2, ::2], output[:, :, 1::2, ::2]]) / 2.
...
class ConditionalResidualBlock(nn.Module):
    def __init__(self, input_dim, output_dim, num_classes, resample=None, act=nn.ELU(),
                 normalization=ConditionalBatchNorm1d, adjust_padding=False, dilation=None, spec_norm=False):
        super().__init__()
...
class ResidualBlock(nn.Module):
    def __init__(self, input_dim, output_dim, resample=None, act=nn.ELU(),
                 normalization=nn.BatchNorm1d, adjust_padding=False, dilation=None, spec_norm=False):
        super().__init__()
        self.non_linearity = act
        self.input_dim = input_dim
        self.output_dim = output_dim
        ...
        if resample == 'down':
            ...
            if dilation is not None:
                ...
                # conv_shortcut = nn.Conv1d ### Something wierd here.
