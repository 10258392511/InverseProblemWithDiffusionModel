...
class NCSN3DShallow(nn.Module):
    def __init__(self, config):
        ...
        self.refine1 = RefineBlock([2 * self.ngf], 2 * self.ngf, act=act, start=True)
        self.refine2 = RefineBlock([2 * self.ngf, 2 * self.ngf], 2 * self.ngf, act=act)
        self.refine3 = RefineBlock([self.ngf, self.ngf], self.ngf, act=act)

        self.conv_temporal_down = nn.Conv3d(2 * self.ngf, 2 * self.ngf, kernel_size=(1, 1, 4), stride=(1, 1, 2), padding=(0, 0, 1))
        self.conv_temporal_up = nn.ConvTranspose3d(2 * self.ngf, self.ngf, kernel_size=(1, 1, 4), stride=(1, 1, 2), padding=(0, 0, 1))
    ...
    def forward(self, x, y):
        ...
        layer1 = self._compute_cond_module(self.res1, output)  # (B, 128, 8, 8, 24)
        layer2 = self._compute_cond_module(self.res3, layer1)  # (B, 256, 8, 8, 24)
        layer3 = self.conv_temporal_down(layer2)  # (B, 256, 8, 8, 12)
        layer4 = self._compute_cond_module(self.res4, layer3)  # (B, 256, 8, 8, 12)
        # down_path = [output, layer1, layer2, layer3, layer4]
        # print(f"down path: {list(map(lambda x: x.shape, down_path))}")

        ref1 = self.refine1([layer4], layer4.shape[2:])  # (B, 256, 8, 8, 12)
        ref2 = self.refine2([layer3, ref1], layer3.shape[2:])  # (B, 256, 8, 8, 12)
        ref3 = self.conv_temporal_up(ref2)  # (B, 256, 8, 8, 24)
        output = self.refine3([layer1, ref3], layer1.shape[2:])  # (B, 256, 8, 8, 24)
        # up_path = [ref1, ref2, ref3, output]
        # print(f"up path: {list(map(lambda x: x.shape, up_path))}")
